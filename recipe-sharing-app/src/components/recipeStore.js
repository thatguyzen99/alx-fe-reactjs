...
const useRecipeStore = create(set => ({
  ...
  addFavorite: (recipeId) => set(state => ({ 
    ...
    recommendations: state.recipes.filter(r => !state.favorites.includes(r.id) && r.id !== recipeId).slice(0, 3)
  })),
  removeFavorite: (recipeId) => set(state => ({
    favorites: state.favorites.filter(id => id !== recipeId),
    // Optionally regenerate recommendations when favorites change
    recommendations: state.recipes.filter(r => !state.favorites.includes(r.id)).slice(0, 3)
  })),
